  ...
  <el-card class="box-card AddNewsContainer">
    <el-form :rules="newsRules" :model="formData" ref="formRef" size="default">
      ...
      <el-form-item label="Type" prop="type">
        <el-select v-model="formData.type" placeholder="Pick a type">
          <el-option
...
const validateType = (rule, value, callback) => {
  if (value.length) {
    ...
    callback(new Error("Must have a title!"));
  }
};
// 校验规则
...
const validateTitle = (rule, value, callback) => {
  if (value.length) {
    ...
    callback(new Error("Must select a type"));
  }
};
// 校验规则
const newsRules = {
